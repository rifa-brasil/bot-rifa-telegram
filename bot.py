...
def webhook():
    try:
        ...
        print(f"📥 RAW WEBHOOK: {raw_data[:400]}...", flush=True)
        ...
        print(f"💬 Texto extraído: '{texto_mensaje}' de {remitente_jid}", flush=True)
        ...
        if es_lista_numeros:
            if estado_actual_rifa in ["finalizada", "bloqueada"]:
                enviar_mensaje_whatsapp(remitente_jid, "⛔ La lista se encuentra cerrada o bloqueada.")
                return jsonify({"status": "ok"}), 200
            ...
            if validos_para_reservar:
                ...
                enviar_mensaje_whatsapp(remitente_jid, f"⏳ Solicitud recibida para: {', '.join([n.zfill(2) for n in validos_para_reservar])}. Total a pagar: {total_a_pagar} reales.")
